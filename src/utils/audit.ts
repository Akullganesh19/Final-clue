...
export function validateAuditLog(log: AuditTrail, previousHash: string): boolean {
  if (log.hashVersion === 2) {
    const expectedHash = generateAuditHashV2(previousHash, log.action, log.details, log.author, log.timestamp);
    return log.hash === expectedHash;
  } else {
    // V1 fallback
    const expectedHash = generateAuditHash(previousHash, log.action, log.details, log.author, log.timestamp);
    return log.hash === expectedHash;
  }
}
