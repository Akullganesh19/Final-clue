...
import { createAuditLog, generateAuditHash, AuditChainConflictError } from './audit';
...
describe('Audit Ledger - Financial Integrity (Ledger)', () => {
  it('should prevent concurrent read-modify-write drifts via OCC', () => {
    ...
    const logsAfterT1 = createAuditLog(initialLogs, 'LOGIN', 'User logged in', 'System', rootHash);
    ...
    assert.throws(
      () => {
        createAuditLog(logsAfterT1, 'ACTION', 'User performed action', 'System', rootHash);
      },
      (err: any) => {
        return err instanceof AuditChainConflictError;
      },
      'Should throw AuditChainConflictError when parent hash diverges'
    );

    // Valid continuation using correct parent hash
    const logsAfterT2 = createAuditLog(logsAfterT1, 'ACTION', 'User performed action', 'System', t1Hash);
